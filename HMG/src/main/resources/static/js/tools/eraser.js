...
function enableEraser(dicomElement) {
	...
	if (eraserEnabled) {
        ...
        const toolState = cornerstoneTools.getToolState(dicomElement, 'Eraser');
...
function multiEnableEraser(element) {
	// 뷰어 요소를 활성화
    ...
    if (toolState && toolState.data && toolState.data.length > 0) {
        console.log(`도구 "Eraser" 활성화 상태 확인 (${element.id}):`, toolState.data);
    } else {
        console.log(`도구 "Eraser" 상태: 비활성화 또는 데이터 없음 (${element.id})`);
    }
}
